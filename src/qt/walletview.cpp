...
{
    bool ok;
    QString privKey = QInputDialog::getText(0, tr(PACKAGE_NAME), tr("Enter a Airin private key to import into your wallet."), QLineEdit::Normal, "", &ok);
    if (ok && !privKey.isEmpty()) {
        ...
        if (!vchSecret.SetString(privKey.toStdString())) {
            QMessageBox::critical(0, tr(PACKAGE_NAME), tr("This doesn't appear to be a Airin private key."));
            return;
        }
