...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the AIRIN money supply statistics") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, airin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AIRIN specific functionality (Masternodes, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
//    strUsage += HelpMessageOpt("-backupzairin=<n>", strprintf(_("Enable automatic wallet backups triggered after each zPiv minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeairinamount=<n>     " + strprintf(_("Keep N AIRIN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 18822, 18824));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The AIRIN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //AIRIN specific: zerocoin and spork DB's
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZSynxBackups = GetBoolArg("-backupzairin", true);
...
//    }
//
//    nAnonymizeAirinAmount = GetArg("-anonymizeairinamount", 0);
//    if (nAnonymizeAirinAmount > 999999) nAnonymizeAirinAmount = 999999;
//    if (nAnonymizeAirinAmount < 2) nAnonymizeAirinAmount = 2;
    ...
    LogPrintf("Anonymize AIRIN Amount %d\n", nAnonymizeAirinAmount);
    ...
    /* Denominations
       ...
       For example:
       1AIRIN+1000 == (.1AIRIN+100)*10
       10AIRIN+10000 == (1AIRIN+1000)*10
    */
